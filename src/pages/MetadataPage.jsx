...
import { fetchCustomers, createCustomer, deleteCustomer } from '../redux/actions';
                    ...
                    filteredCustomers.map((customer) => (
                        <div key={customer.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
                            ...
                            <div>
                                ...
                                {customer.recentWorkOrders && customer.recentWorkOrders.length > 0 ? (
                                    <div className="space-y-1">
                                        {customer.recentWorkOrders.map((wo, index) => (
                                            <div key={index} className="flex justify-between items-center">
                                                <span className="text-xs text-blue-600 font-medium">{wo.id}</span>
                                                <span className="text-xs text-gray-600">
                                                    {wo.status} - RF {wo.amount}
                                                </span>
                                            </div>
                                        ))}
                                    </div>
